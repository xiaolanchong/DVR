...
to exploit new features:
	...
	void SetTextColor(const COLORREF clrText, const BOOL bRedraw=TRUE)
	COLORREF GetTextColor() const

	void SetBkColor(const COLORREF clrBack, const BOOL bRedraw=TRUE)
...
private:
	// flag that notifies that edit control was created using Create function 
	// (not subclassed)
	BOOL m_bCreated;
	// flag that notifies that edit control was completely initialized
	BOOL m_bInitialized;
...
public:
	...
	BOOL SetGroupSeparator(const TCHAR chGroupSeparator);
	...
	BOOL SetGroupLength(const int nGroupLength);
	...
	BOOL SetDecimalSeparator(const TCHAR chDecimalSeparator);
	...
	//				leading zero or not
	void SetShowLeadingZero(const BOOL bShowLeadingZero);
	...
	BOOL SetFractionalDigitCount(const int nFractionalDigitCount);
	...
	BOOL SetDecimalDigitCount(const int nDecimalDigitCount);
	...
	void SetNegativeTextColor(const COLORREF clrTextNegative, const BOOL bRedraw=TRUE);
	...
	//				all the time or not
	void SetForceToUsePeriod(const BOOL bForceToUsePeriod);
