	...
	mDefaultTime( boost::gregorian::date( 1970,1,1 ), boost::posix_time::time_duration( 0,0,0 ) )
{
	...
	//Pens
	mPens[SCP_TrackerPen] = wxPen( wxColour( 0, 0, 0 ) );
...
void ScheduleControl::OnPaint( wxPaintEvent& event )
{
	...
	// eugene add-on
	wxColour clBackgound = wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE);

	dc.SetBackground( wxBrush(/* wxColour( 180,180,180 )*/clBackgound ) );
	dc.Clear();
...
void ScheduleControl::DrawClock( wxDC& dc )
{
	...
	dc.GetTextExtent( timePart.c_str(), &textWidth, &textHeight  );
	...
	dc.GetTextExtent( datePart.c_str(), &textWidth, &textHeight  );
	clockXOffset = ( mDimensions[SCD_DateTimeWidth] - textWidth ) / 2;
	clockYOffset = ( mDimensions[SCD_HeaderHeight] - textHeight ) / 2 + 4;
...
void ScheduleControl::DrawTracker( wxDC& dc )
{
	...
	dc.SetBrush( */*wxRED_BRUSH*/wxBLACK_BRUSH );
