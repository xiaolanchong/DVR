...
//////////////////////////////////////////////////////////////////////////
//!\brief Расширение CIAnalyzer для использования в проекте DVR
//!
//! Интерфейс получается функцией GetAnalyzerInterfaceExt, если указать
//! флаг GAIE_DVR_EXT
//////////////////////////////////////////////////////////////////////////

class CIAnalyzerDVR : public CIAnalyzer {
public:
	/*! \brief Получить маску движения Frame::label для последнего обработанного кадра
		
		\param labels - буфер с маской
		\param width - ширина кадра
		\param height - высота кадра
		\return 0, если нет ошибки

		Список значений для меток:

		0 - проверка на движение не проводилась;
		1-200 - область движения, ассоциированная с некоторым объектом;
		0xFF - точка проверялась, и движение в ней нет
		0xFA,0xF9 - движущиеся точки, которые в конце концов были отклонены из-за несоответствия по размеру области движения
		
		0xF8 - точки "световой ауры" для алгоритма отсечения фар, сейчас не используется

		0xFC - точки засветки (сейчас не ставятся)
		0xF9 - точки тени (сейчас не ставятся)
		
		Необходимо иметь ввиду, значения меток могут изменятся разработчиками алгоритмов.

	*/
	virtual long GetLatestMotionLabels(char*& labels, long& width, long& height)=0;
};
